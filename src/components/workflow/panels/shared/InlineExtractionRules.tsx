...
interface InlineExtractionRulesProps {
  extractions: ExtractionRule[];
  onChange: (extractions: ExtractionRule[]) => void;
}

export const InlineExtractionRules: React.FC<InlineExtractionRulesProps> = ({ extractions = [], onChange }) => {
  const addExtraction = () => {
      ...
      {extractions.map((extraction, index) => (
        <div key={index} className="border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Input
              placeholder="使用 JMESPath 语法，例如：body.code 或 body.data[0].orderType"
