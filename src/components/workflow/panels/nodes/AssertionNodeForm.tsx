...
import type { AssertionConfig, AssertionRule } from '../../types';

interface AssertionNodeFormProps {
  config: AssertionConfig;
  onChange: (config: AssertionConfig) => void;
}

export const AssertionNodeForm: React.FC<AssertionNodeFormProps> = ({ config, onChange }) => {
  const rules = config.rules || [];

  const updateConfig = (updates: Partial<AssertionConfig>) => {
          ...
          {rules.map((rule, index) => (
            <div key={index} className="border border-gray-200 rounded-lg p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <Input
                  placeholder="body.code 或 body.data[0].companyName 或 status_code"
                  value={rule.source || ''}
