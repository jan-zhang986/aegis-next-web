/**
 * 缺陷原因选择器：非必填、单选，一级展开/收起，二级按列排布
 * 搜索在触发区输入框内完成，不单独开搜索框
...
import { BUG_DEFECT_REASON_GROUPS } from '@/services/bug-management/constants/bug-defect-reason';
import type { FeishuDefectReasonGroup } from '@/services/bug-management/service';
import { cn } from '@/utils/cn';

const DEFAULT_GROUPS = BUG_DEFECT_REASON_GROUPS;
...
interface DefectReasonSelectProps {
    value: string;
    onChange: (value: string) => void;
    /** 从飞书接口拉取的缺陷原因选项（与业务线一样）；未传或空时使用写死常量 */
...
export function DefectReasonSelect({
    value,
    onChange,
    groups: groupsProp,
    placeholder = '待填',
