...
import {
    ...
    CheckCircle, XCircle, Clock, AlertTriangle, Workflow,
...
export function PlanDetailScenarioCase({ planId, projectId, canEdit, onRefresh, embedInPlanTree, defaultModuleId }: PlanDetailScenarioCaseProps) {
    ...
    const fetchModuleTree = useCallback(async () => {
        try {
            const res = await testPlanManagementService.getApiScenarioModule({
                testPlanId: planId,
                treeType: 'MODULE'
            });
            setModuleTree(res || []);
    ...
    const fetchCaseList = useCallback(async () => {
        ...
        try {
            ...
            const res = await testPlanManagementService.getPlanDetailApiScenarioList(params);
                ...
                <Table className="text-sm">
                    <TableHeader className="bg-[#f7f8fa] sticky top-0 z-10 border-b border-gray-100">
                        <TableRow className="hover:bg-transparent border-none h-9">
                            ...
                            <TableHead className="min-w-[200px] font-medium text-gray-500 text-xs">场景名称</TableHead>
