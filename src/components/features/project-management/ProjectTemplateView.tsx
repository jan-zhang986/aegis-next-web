...
} from '@/components/features/system-setting/template';
...
export function ProjectTemplateView({ projectId }: ProjectTemplateViewProps) {
    ...
    if (subView === 'field') {
        return <TemplateFieldSetting scopeId={projectId} scope="project" scene={scene} onBack={onBack} />;
    }
    if (subView === 'templates') {
        return <TemplateManagementList scopeId={projectId} scope="project" scene={scene} onBack={onBack} />;
    }
    if (subView === 'workflow') {
        return <TemplateWorkflowView scopeId={projectId} scope="project" scene={scene} onBack={onBack} />;
    }
    ...
    return (
        <div className="space-y-8 max-w-[1400px] mx-auto animate-in fade-in duration-700 pb-12">
            <div className="flex flex-col gap-1 px-1">
                ...
                <p className="text-sm font-medium text-gray-400 mt-2 pl-14">统一管理项目内各核心模块的自定义字段、用例/缺陷模板及业务流转规则。</p>
            </div>
