...
1. **高**：测试报告详情完整版（执行分析 + 用例/缺陷明细 + 头部），否则报告可读性不足。
2. **中**：报告导出 PDF、报告分享（链接 + 分享详情页）。
3. **中**：报告列表增强（类型筛选、批量删除、重命名、单条导出）。
4. **低**：自定义配置报告（若产品需要可配置报告模板）。
5. **已完成**：场景用例 Tab 已由 ComingSoon 替换为 PlanDetailScenarioCase。
