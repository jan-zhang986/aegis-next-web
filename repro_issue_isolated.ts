...
// THE FUNCTION TO TEST (with fix applied)
function truncateToPreconditionOnly(content: string, allowNumberedList = false): string {
    ...
    for (const line of lines) {
        ...
        // Fix applied here
        if (!allowNumberedList && /^\d+[.)、]\s+\S/.test(trimmed)) break;
        result.push(line);
    }
    let out = result.join('\n').trim();
    if (out === '无') return '';
...
const inputs = [
  ...
  **前置条件**
  ...
  featureCaseEnd
  `,
    // Case 4: Explicit H3 precondition with extra text
    `
  featureCaseStart
  ## Case 4
  ### 前置条件
